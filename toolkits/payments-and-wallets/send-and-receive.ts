...
async function main() {
  ...
  console.log("Recipient balance before:", recipientAccount.parsed.amount.toString());

  // === SEND: Transfer tokens ===
  const sourceAta = getAssociatedTokenAddressInterface(mint, sender.publicKey);
  const destinationAta = getAssociatedTokenAddressInterface(mint, recipient.publicKey);
  ...
  const senderFinal = await getOrCreateAtaInterface(rpc, payer, mint, sender.publicKey);
  const recipientFinal = await getOrCreateAtaInterface(rpc, payer, mint, recipient.publicKey);
